...
| `run_mission_conflict.py` | Mission-changing | Run both strategic coalition workers until the current DCS mission ends, with manual or automatic war declaration. |
...
| `run_navigation_menu.py` | In-game menu / Overlay / SRS | Show/hide the ME route, request navigation or Flight status, enable/disable hints, browse Navaids or Airfields / ATC, and run Hound/MSRS Copilot tone, TTS and sender-queue checks through the general radio arbiter. Flight status includes optional FLIGHTGROUP FSM, altitude/weather, IAS/TAS/GS/Mach, and MAG/TRUE directions in a grouped 15-second readout. Lists initialize once per new group menu; Refresh nearby updates ordering later. Airfield radio data is joined to live MOOSE AIRBASE objects by numeric ID and never tunes the cockpit. Navaids and Airfields / ATC require the local importer cache but no FLIGHTGROUP. Replaces the test menu for this run. |
...
| `generate_strategic_objectives.py` | Read-only DCS | Generate and inspect scoped objectives from territories and normalized infrastructure. |
...
| `test_capture_reaction.py` | Mission-changing | Capture one configured OPSZONE, confirm its persistent combat patrol, and validate that the opponent selects a feasible recapture or defers it with a concrete asset shortfall. |
